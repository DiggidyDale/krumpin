...
func newSkillWidget(skill *Skill, parentWindow fyne.Window) *fyne.Container {
    rollButton := widget.NewButton(skill.Name, func() {
        if skill.DiceVal == "" {
            dialog.ShowInformation("Dice Roll", fmt.Sprintf("No dice selected for %s.", skill.Name), parentWindow)
            return
        }
        ...
        showRollResultDialog(result, maxSides, skill.Name, parentWindow)
    })
    ...
    if skill.Description != "" {
        infoButton = NewInfoCircle(skill.Description, parentWindow)
    } else {
...
func loadSkillsFromFile(fileName string) ([]*Skill, error) {
    ...
    for scanner.Scan() {
        ...
        if line != "" {
            ...
            skill := &Skill{Name: parts[0], DiceVal: diceOptions[0], Description: parts[1]}
...
func main() {
    ...
    skills, _ := loadSkillsFromFile("resources/skills.txt")

    skillsContainer := container.NewVBox()
    skillsTitle := widget.NewLabelWithStyle("Skills (Click skill name to roll dice):", fyne.TextAlignLeading, fyne.TextStyle{Bold: true})
